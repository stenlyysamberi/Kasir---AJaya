...
<div class="row">
    ...
    if (isset($_POST['keyword'])) {
        ...
        $product = mysqli_query($koneksi,"SELECT tbl_satuan_produk.nama_satuan, tbl_barcode_barang.*, tbl_barang.harga FROM tbl_barcode_barang INNER JOIN tbl_barang ON tbl_barang.id_barcode = tbl_barcode_barang.id_barcode INNER JOIN tbl_satuan_produk ON tbl_barcode_barang.id_satuan = tbl_satuan_produk.id_satuan WHERE tbl_barcode_barang.join_ket = 'Berhasil' AND tbl_barcode_barang.code_barcode LIKE '%$cari%' OR tbl_barcode_barang.nama_barang LIKE '%$cari%' ORDER BY tbl_barcode_barang.id_barcode DESC");
    }else{

        // $product = mysqli_query($koneksi,"SELECT * FROM tbl_barcode_barang WHERE join_ket='Berhasil'
        //     ORDER BY id_barcode DESC");

        $product = mysqli_query($koneksi,"SELECT tbl_satuan_produk.nama_satuan, tbl_barcode_barang.*, tbl_barang.harga FROM tbl_barcode_barang INNER JOIN tbl_barang ON tbl_barang.id_barcode = tbl_barcode_barang.id_barcode INNER JOIN tbl_satuan_produk ON tbl_barcode_barang.id_satuan = tbl_satuan_produk.id_satuan WHERE tbl_barcode_barang.join_ket = 'Berhasil' ORDER BY tbl_barcode_barang.id_barcode DESC");
            ...
            <div class="card-box product-box">
                ...
                <div class="product-info">
                    <div class="row align-items-center">
                        <div class="col">
                            <h5 class="font-16 mt-0 sp-line-1"><a href="ecommerce-product-detail.html" class="text-dark"><?php echo $code['nama_barang']?> . <?php echo "Rp.". number_format($code['harga'])?></a> </h5>
